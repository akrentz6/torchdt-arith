...
parser = argparse.ArgumentParser(description="Train MobileNetV1 on CIFAR10 in LNS")
...
date = datetime.now()
log_filename = f"mobilenetv1_lns_train_{date.strftime('%Y%m%d_%H%M%S')}.log"
history_filename = f"mobilenetv1_lns_history_{date.strftime('%Y%m%d_%H%M%S')}.json"

save_dir = "./checkpoints"
os.makedirs(save_dir, exist_ok=True)

best_model_filename = os.path.join(save_dir, f"mobilenetv1_lns_best_{date.strftime('%Y%m%d_%H%M%S')}.pt")
final_model_filename = os.path.join(save_dir, f"mobilenetv1_lns_final_{date.strftime('%Y%m%d_%H%M%S')}.pt")
...
model = MobileNetV1(10, madam=(args.optimizer == "madam"), dtype=dtype, device=device)
...
logger.info(f"Training MobileNetV1 on CIFAR-10 with LNS64 (f={prec}, lookup table={table}) on device {device}")
if args.optimizer == "sgd":
