...
parser = argparse.ArgumentParser(description="Train MobileNetV1 on CIFAR10 in FP")
...
log_filename = f"mobilenetv1_fp_train_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
history_filename = f"mobilenetv1_fp_history_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"

save_dir = "./checkpoints"
os.makedirs(save_dir, exist_ok=True)

best_model_filename = os.path.join(save_dir, f"mobilenetv1_fp_best_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pt")
final_model_filename = os.path.join(save_dir, f"mobilenetv1_fp_final_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pt")
...
model = MobileNetV1(10, dtype=dtype, device=device)

criterion = nn.NLLLoss()
optimizer = SGD(model.parameters(), lr=lr, momentum=momentum, weight_decay=weight_decay)
scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[50, 75], gamma=0.1)

logger.info(f"Training MobileNetV1 on CIFAR-10 with FP (dtype={dtype}) on device {device}")
